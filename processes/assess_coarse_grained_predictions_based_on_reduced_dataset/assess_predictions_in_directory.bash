...
echo "Assessing $INDIR as $MOLCLASS"
...
do
	cat "$PREDFILE" | awk '{print ($1*0.1597586)+0.2234375}' | sponge "$PREDFILE"
done
...
| while read ID
do
	cat "../generate_coarse_grained_graphs_including_trajectory_data/output/graphs/${MOLCLASS}/${ID}_nodes.csv" | tr ',' ' ' | awk '{print $NF}' | tail -n +2 > "${OUTDIR}/groundtruth/${ID}"
done
...
| while read ID
do 
	cat "../generate_coarse_grained_graphs_including_trajectory_data/output/graphs/${MOLCLASS}/${ID}_nodes.csv" | tr ',' ' ' | grep -v ground \
